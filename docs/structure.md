...
モータにはロータリーエンコーダというセンサがついています。
このセンサはモータがどれぐらい回転したかを検出することができます。
...
モータコントローラと`LiDAR`は小型コンピュータである`Raspberry Pi 4`に接続されています。
...
`Raspberry Pi 4`には操作用に無線ジョイスティックと無線キーボードが接続されています。
無線キーボードにはタッチパッドがあり、指をスライドすればマウスカーソルが移動し、タッチパッド下部のボタンはマウスの左右ボタンのクリックと同じように使えます。
