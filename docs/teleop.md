...
デスクトップ画面にある`T.op`（テレオペレーション：ロボットの手動操作）というボタンをダブルクリックしてください。
...
もしも、カメラ画像が表示されない場合は、デスクトップ画面下の方にある`Stop`というボタンを１回クリックし、ソフトを終了させてからもう一度`T.op`をクリックしてください。

![2025-03-08_175253.png](../images/2025-03-08_175253.png)

３次元表示の部分にマウスカーソルを移動させ、タッチパッドの左ボタンを押しながら指をスライドすると画面を様々な方向から眺めることができます。
